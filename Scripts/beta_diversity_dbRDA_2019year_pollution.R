...
Beta_diversity <- function(data,name)
{
  ...
  nonmetric_label = list(line_1,line_3)
  ...
  p1 <- ggplot(data = pcoa, aes(x=PC1,y=PC2))+#,color=pcoa$Site,shape = pcoa$Season))+
    ...
    geom_text(data=fit.df.1,aes(x=PC1* arrow_factor * 1.1,y=PC2* arrow_factor* 1.1,label=pollution),size=3,colour="#EEAD0E")+
    ...
    geom_text(data=taxon_fit.df.1,aes(x=PC1* taxon_arrow_factor,y=PC2* taxon_arrow_factor,label=microbes),size=3,
              color = "grey") +
  ...
  merge_tab_2 <- merge_tab_1 %>%
    `rownames<-`(rownames(taxon)) #%>%
    #dplyr::select(R_sign_table$name)
  #paste0("taxon ~ ",)
  set.seed(123)
  ...
  set.seed(123)
  db_rda_forward_pr <- ordiR2step(capscale(taxon~1, merge_tab_2, distance = 'bray'), scope = formula(db_rda), R2scope = T, direction = 'forward', permutations = 999)
  ...
  line_5 = bquote("Constrained explained variation:"~ R^2 ~"= "~.(paste(round(db_rda_forward_pr_Radj*100, 2), '%')))
  #nonmetric_label = list(line_1,line_2,line_3,line_4,line_5)
...
###Pcoa plots  -----------------------------------------------------------------
library(patchwork)
pdf("~/Desktop/marine_sediment/manuscript/Figure_v5/Pcoa_taxon_2019.pdf",
    width = 10,height = 60)
Phylum_list$pcoa +  Class_list$pcoa +
  Order_list$pcoa + Family_list$pcoa +
  Genus_list$pcoa + Species_list$pcoa +
  plot_layout(ncol = 1)
dev.off()
#### Phylum level --------------------------------------------------------------------
bar_plot <- Barplot(Phylum_list$cumRdf,Phylum_list$levels)
Phylum_list$dbRDA_withbarplot <- Phylum_list$dbRDA + bar_plot + plot_layout(design = "AAAB")
#### Class level --------------------------------------------------------------------
bar_plot <- Barplot(Class_list$cumRdf,Class_list$levels)
Class_list$dbRDA_withbarplot <- Class_list$dbRDA + bar_plot + plot_layout(design = "AAAB")
#### Order level --------------------------------------------------------------------
bar_plot <- Barplot(Order_list$cumRdf,Order_list$levels)
Order_list$dbRDA_withbarplot <- Order_list$dbRDA + bar_plot + plot_layout(design = "AAAB")
#### Family level --------------------------------------------------------------------
bar_plot <- Barplot(Family_list$cumRdf,Family_list$levels)
Family_list$dbRDA_withbarplot <- Family_list$dbRDA + bar_plot + plot_layout(design = "AAAB")
#### Genus level ---------------------------------------------------------------
bar_plot <- Barplot(Genus_list$cumRdf,Genus_list$levels)
Genus_list$dbRDA_withbarplot <- Genus_list$dbRDA + bar_plot + plot_layout(design = "AAAB")
#### Species level -------------------------------------------------------------
...
pdf("~/Desktop/marine_sediment/manuscript/Figure_v5/FigS2_Pcoa_genus.pdf",
    width = 10,height = 10)
beta_diversity_dbRDA_list$Genus$pcoa
dev.off()
...
# dev.off()
pdf("~/Desktop/marine_sediment/results/dbRDA_taxon_2019/dbRDA_taxon_2019_phylum.pdf",width = 16,height = 10)
beta_diversity_dbRDA_list$Phylum$dbRDA_withbarplot
dev.off()
pdf("~/Desktop/marine_sediment/results/dbRDA_taxon_2019/dbRDA_taxon_2019_class.pdf",width = 16,height = 10)
beta_diversity_dbRDA_list$Class$dbRDA_withbarplot
dev.off()
pdf("~/Desktop/marine_sediment/results/dbRDA_taxon_2019/dbRDA_taxon_2019_order.pdf",width = 16,height = 10)
beta_diversity_dbRDA_list$Order$dbRDA_withbarplot
dev.off()
pdf("~/Desktop/marine_sediment/results/dbRDA_taxon_2019/dbRDA_taxon_2019_family.pdf",width = 16,height = 10)
beta_diversity_dbRDA_list$Family$dbRDA_withbarplot
dev.off()
pdf("~/Desktop/marine_sediment/results/dbRDA_taxon_2019/dbRDA_taxon_2019_genus.pdf",width = 16,height = 10)
beta_diversity_dbRDA_list$Genus$dbRDA_withbarplot
dev.off()
pdf("~/Desktop/marine_sediment/manuscript/Figure_v2/dbRDA_taxon_2019_species.pdf",width = 16,height = 10)
#beta_diversity_dbRDA_list$Species$dbRDA_withbarplot
Species_list$dbRDA_withbarplot
dev.off()

pdf("~/Desktop/marine_sediment/manuscript/manuscripts/Marine_microbiome_v3/Figures_v3/dbRDA_taxon_2019_species.pdf",width = 18,height = 10)
#beta_diversity_dbRDA_list$Species$dbRDA_withbarplot
Species_list$dbRDA_withbarplot
dev.off()
