...
#   filter(name %in% COG_list$COG_pathway_list$re_df_01$name)
## Plot sankey -----------------------------------------------------------------
# re_df <- readRDS("~/Projects/Marine_Sediment/mediation/mediation/pollution_microbiome_benthos_neg.rds")
# re_df_tidy <- re_df %>%
...
re_df_tidy <- read_tsv("~/Projects/Marine_Sediment/mediation/mediation_new/mediation_species.tsv")
...
links_color1 <- links_color %>%
 ...
 Color_df <- data.frame(name = Node) %>%
  left_join(Pollution_group) %>%
  left_join(Pollution_group_palette) %>%
  mutate(color = if_else(name %in% Microbiome_feature,"#0072B2",color)) %>%
  mutate(color = if_else(name %in% Microbiome_feature_pos_with_high_pollute,"#D55E00",color)) %>%
  ...
  mutate(group = if_else(name %in% c(Microbiome_feature,Microbiome_feature_pos_with_high_pollute),"microbiome",group)) %>%
  mutate(group = if_else(name %in% c(Microbiome_feature,Small_animal_feature_pos),"Benthos",group))
fig <- plot_ly(
  type = "sankey",
  orientation = "h",
