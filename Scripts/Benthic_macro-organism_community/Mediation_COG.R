...
pollution_mat <- data.frame("SampleID" = rownames(Microbiome_species_otu)) %>%
  ...
  normalize(.)
## Mediation Analyses  pollution -> COG -> animal ----------------------------------------------------------
...
re_df <- foreach (i = 1:dim(pollution_mat)[2],.combine=rbind) %dopar% {
  ...
  for (j in 1:length(cog_path_mat)) {
    #j <- 1
    for (k in 1:length(small_animal_mat_neg)) {
      #k <- 17
      b = colnames(cog_path_mat)[j]
...
saveRDS(re_df,"~/Projects/Marine_Sediment/mediation/mediation_new/pollution_COG_benthos_neg.rds")
...
re_df_1 <- foreach (i = 1:dim(pollution_mat)[2],.combine=rbind) %dopar% {
  ...
  for (j in 1:length(small_animal_mat_neg)) {
    #j <- 1
    for (k in 1:length(cog_path_mat)) {
      #k <- 17
      b = colnames(small_animal_mat_neg)[j]
      c = colnames(cog_path_mat)[k]
      #str = paste0(a,"_",b,"_",c)
      #print(str)
      mediation_tmp <- func_main(ALl_mat_neg,a,b,c)
      mediation_df <- rbind(mediation_df,mediation_tmp)
...
saveRDS(re_df_1,"~/Projects/Marine_Sediment/mediation/mediation_new/pollution_benthos_neg_COG.rds")
re_df_1 <- readRDS("~/Projects/Marine_Sediment/mediation/mediation_new/pollution_benthos_neg_COG.rds")
re_df_tidy1 <- re_df_1 %>%
  mutate(BH_Beta_step2_iv = p.adjust(Beta_step2_iv_p)) %>%
  mutate(BH_Beta_step3_med = p.adjust(Beta_step3_med_p)) %>%
  #filter(BH_Beta_step2_iv < 0.25,BH_Beta_step3_med < 0.25) %>%
...
write_tsv(re_df_tmp,"~/Projects/Marine_Sediment/mediation/mediation_new/mediation_COGs.tsv")
# re_df_tidy_pos_microbiome <- re_df_tidy %>%
#   filter(Beta_step2_iv > 0)
# write_tsv(re_df_tidy_pos_microbiome,"~/Projects/Marine_Sediment/mediation/mediation/pollution_COG_benthos_neg_microbiome_pos.tsv")
# re_df_tidy_neg_microbiome <- re_df_tidy %>%
#   filter(Beta_step2_iv < 0)
# write_tsv(re_df_tidy_neg_microbiome,"~/Projects/Marine_Sediment/mediation/mediation/pollution_COG_benthos_neg_microbiome_neg.tsv")


# ## pollution -> COG -> animal (pos) -------------------------------------------
# 
# registerDoParallel(22)
# 
# re_df <- foreach (i = 1:dim(pollution_mat)[2],.combine=rbind) %dopar% {
...
# }
# 
# saveRDS(re_df,"~/Projects/Marine_Sediment/mediation/mediation/pollution_microbiome_COG_pos.rds")
# re_df <- readRDS("~/Projects/Marine_Sediment/mediation/mediation/pollution_microbiome_COG_pos.rds")
# 
# re_df_tidy <- re_df %>%
#   mutate(BH_Beta_step2_iv = p.adjust(Beta_step2_iv_p)) %>%
#   mutate(BH_Beta_step3_med = p.adjust(Beta_step3_med_p)) %>%
...
#   filter(Beta_step2_iv > 0)
# write_tsv(re_df_tidy_pos_microbiome_pos_animal,"~/Projects/Marine_Sediment/mediation/mediation/pollution_COG_benthos_pos_microbiome_pos.tsv")
# re_df_tidy_neg_microbiome_pos_animal <- re_df_tidy %>%
#   filter(Beta_step2_iv < 0)
# write_tsv(re_df_tidy_neg_microbiome_pos_animal,"~/Projects/Marine_Sediment/mediation/mediation/pollution_COG_benthos_pos_microbiome_neg.tsv")
# ## Mediation Analyses  pollution -> animal -> COG ----------------------------------------------------------
...
#   arrange(ID) %>%
#   column_to_rownames("ID")
# cog_path_mat_neg <- COG_pathway %>%
#   dplyr::select(rowname,COG_pathway_feature$name) %>%
#   rename(ID = rowname) %>%
#   arrange(ID) %>%
#   column_to_rownames("ID")
# cog_path_mat_pos <- COG_pathway %>%
#   dplyr::select(rowname,COG_pathway_feature_pos_with_high_pollute$name) %>%
#   rename(ID = rowname) %>%
#   arrange(ID) %>%
#   column_to_rownames("ID")
...
#   column_to_rownames("ID")
# 
# ALl_mat_neg <- cbind(microbiome_mat,cog_path_mat_neg,small_animal_mat,pollution_mat)
# ALl_mat_pos <- cbind(microbiome_mat,cog_path_mat_pos,small_animal_mat,pollution_mat)
# 
# library(foreach)
# library(doParallel)
...
#     #j <- 1
#     for (k in 1:length(cog_path_mat_neg)) {
#       #k <- 17
#       b = colnames(small_animal_mat)[j]
#       c = colnames(cog_path_mat_neg)[k]
#       #str = paste0(a,"_",b,"_",c)
#       #print(str)
#       mediation_tmp <- func_main(ALl_mat_neg,a,b,c)
#       mediation_df <- rbind(mediation_df,mediation_tmp)
...
# }
# 
# saveRDS(re_df,"~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_neg.rds")
# re_df <- readRDS("~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_neg.rds")
# re_df_tidy <- re_df %>%
#   mutate(BH_Beta_step2_iv = p.adjust(Beta_step2_iv_p)) %>%
#   mutate(BH_Beta_step3_med = p.adjust(Beta_step3_med_p)) %>%
#   #filter(BH_Beta_step2_iv < 0.25,BH_Beta_step3_med < 0.25) %>%
...
#   filter(Beta_step2_iv > 0)
# write_tsv(re_df_tidy_pos_animal_neg_microbiome,"~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_neg_benthos_pos.tsv")
# re_df_tidy_neg_animal_neg_microbiome <- re_df_tidy %>%
#   filter(Beta_step2_iv < 0)
# write_tsv(re_df_tidy_neg_animal_neg_microbiome,"~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_neg_benthos_neg.tsv")
# 
...
#   for (j in 1:length(small_animal_mat)) {
#     #j <- 1
#     for (k in 1:length(cog_path_mat_pos)) {
#       #k <- 17
#       b = colnames(small_animal_mat)[j]
#       c = colnames(cog_path_mat_pos)[k]
#       #str = paste0(a,"_",b,"_",c)
#       #print(str)
#       mediation_tmp <- func_main(ALl_mat_pos,a,b,c)
#       mediation_df <- rbind(mediation_df,mediation_tmp)
...
# }
# 
# saveRDS(re_df,"~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_pos.rds")
# re_df <- readRDS("~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_pos.rds")
# re_df_tidy <- re_df %>%
#   mutate(BH_Beta_step2_iv = p.adjust(Beta_step2_iv_p)) %>%
#   mutate(BH_Beta_step3_med = p.adjust(Beta_step3_med_p)) %>%
#   #filter(BH_Beta_step2_iv < 0.25,BH_Beta_step3_med < 0.25) %>%
...
#   filter(Beta_step2_iv > 0)
# write_tsv(re_df_tidy_pos_animal_pos_microbiome,"~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_pos_benthos_pos.tsv")
# re_df_tidy_neg_animal_pos_microbiome <- re_df_tidy %>%
#   filter(Beta_step2_iv < 0)
# write_tsv(re_df_tidy_neg_animal_pos_microbiome,"~/Projects/Marine_Sediment/mediation/mediation/pollution_benthos_COG_pos_benthos_neg.tsv")
# 
